...
public class Implementaion2 {
	static List<String> returnValue = new ArrayList<>();
	static List<HelperModel> holder = new CopyOnWriteArrayList<>();
	static BufferedReader br = null;
	static String line = "";
	static String cvsSplitBy = ",";
	static String outgoingName = null;
	...
	public static void Start() throws IOException {
		...
		for (String csvFileName : file) {
			...
			if (newFileMatcher.find()) {
				...
				Files.deleteIfExists(Paths.get("./" + newFileSaver + ".CDR"));
			...
			try {
				...
				while ((line = br.readLine()) != null) {
					...
					Matcher UserMatcher = UserPattern.matcher(line.split(cvsSplitBy)[5]);
					...
					if (UserMatcher.find()) {
						outgoingName = UserMatcher.group(2);
					}
					if (TypeMatcher.find()) {
					...
					holder.add(new HelperModel(datee, Duration, outgoingName, type));
				}
				fw = new FileWriter("./" + newFileSaver + "_OUTGOING.CDR");
				...
				if (br != null) {
					...
					for (int i = 0; i < holder.size() - 1; i++) {
						for (int j = i + 1; j < holder.size(); j++) {
							if (holder.get(i).getUser().contains(holder.get(j).getUser())) {
								temp = Long.parseLong(holder.get(i).getDuration())
										+ Long.parseLong(holder.get(j).getDuration());
					...
					for (HelperModel hold : holder) {
						list.add(hold.getDate() + ";" + hold.getDuration() + ";" + hold.getUser() + ";" + hold.getType()
								+ "_OUT" + ";;;;;;;;;;;;;;;;;;; \n");
					}
			...
			returnValue.add("./" + csvFileName);
		}
		for (String ttt : returnValue) {
			Files.deleteIfExists(Paths.get(ttt));
		}
	}
}
